...
public class ExamsController {
    ...
    @ApiOperation("报名考试")
    @PostMapping("/registeExams/{subject}")
    public Result registeExams(@RequestHeader("token") String token, @PathVariable String subject) {
    ...
    public Result startExams(@RequestHeader("token") String token, @PathVariable String subject) {
        ...
        if (exams.getStatus() == 1) {
            return Result.fail("考试已经开始");
        }
